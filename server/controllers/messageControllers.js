...
export const getUserForSidebar = async() =>{
    try{
        ...
        constfilterUsers = await User.find({_id:{$ne:userId}}).select("-password");
        ...
        res.json({success:true,users:filterUsers,unseenMessages:unseenMessages})
    }catch(error){
...
//get all messages foe selected user
