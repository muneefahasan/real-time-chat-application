...
if(user){
    return res.json({success:false,message:"Account is already exist"})

}
const salt - awit bcrypt.genSalt(10);
...
res.json({success:true,userData:newUser,token,message:"Account 
...
res.json({success:true,userData:newUser,token,message:"Login successful"})
    ...
    expoort const updateProfile = async (req,res) =>{
        try{
